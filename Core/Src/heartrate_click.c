...
/************************************************************************************
 * Heartrate_click functions
 ************************************************************************************/
void HAL_I2C_MasterRxCpltCallback(I2C_HandleTypeDef *hi2c) {
	temperature = 15;
	uint8_t buffer[4] = {0};
	heartrate1_i2c_hal_read(FIFO_DATA_REG, 1, &buffer[0]);
	heartrate1_i2c_hal_read(FIFO_DATA_REG, 1, &buffer[1]);
	heartrate1_i2c_hal_read(FIFO_DATA_REG, 1, &buffer[2]);
	heartrate1_i2c_hal_read(FIFO_DATA_REG, 1, &buffer[3]);

	HAL_I2C_Master_Receive_IT(hi2c, SlaveReadAddress, i2c_rxbuf, 8);
}
...
void Heart_Rate_Init() {
	...
	HAL_I2C_Master_Receive_IT(&hi2c1, SlaveReadAddress, i2c_rxbuf, 8);
}
...
void heartrate1_i2c_hal_write(uint8_t address, uint16_t num, uint8_t *buff) {
	...
	HAL_StatusTypeDef ret = HAL_I2C_Master_Transmit(&hi2c1, SlaveWriteAddress,
			buffer, num + 1, 1000);
...
void heartrate1_i2c_hal_read(uint8_t address, uint16_t num, uint8_t *buff) {
	...
	HAL_StatusTypeDef ret1 = HAL_I2C_Master_Transmit(&hi2c1, SlaveWriteAddress,
			&address, 1, 1000);
	HAL_StatusTypeDef ret2 = HAL_I2C_Master_Receive(&hi2c1, SlaveReadAddress,
			buff, num, 1000);
}
